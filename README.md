...
A simple REST API server for arithmetic calculations with JWT authentication, built with FastAPI and SQLModel.
...
All settings are configurable via environment variables with the `CALC_API_` prefix:
...
Supported operations for creating calculations: `add`, `sub`, `mul`, `div`.
