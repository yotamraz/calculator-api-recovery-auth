...
Generated at: 2026-02-17T19:42:35.658840+00:00
...
TEST_CASES: list[dict[str, Any]] = resolve_env_placeholders(
    json.loads(r'''[
    {
        ...
        "endpoint": "/health",
        "method": "GET",
        "description": "Verify the health endpoint returns status ok and version 0.1.0",
        "request_data": {
            "path": {},
            "query": {},
            "body": null
    ...
    },
    {
        ...
        "method": "POST",
        "description": "Register a new user and verify the response contains id, username, and created_at",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    {
        "name": "register_missing_password",
        "category": "MISSING_REQUIRED",
        "endpoint": "/auth/register",
        "method": "POST",
        "description": "Attempt to register a user without providing a password, expect 422 validation error",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "endpoint": "/auth/token",
        "method": "POST",
        "description": "Login with valid credentials using OAuth2 form and receive an access token",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "endpoint": "/auth/token",
        "method": "POST",
        "description": "Attempt login with incorrect password, expect 401 Unauthorized",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "endpoint": "/add",
        "method": "POST",
        "description": "Add two numbers with a valid auth token and verify the result",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "endpoint": "/add",
        "method": "POST",
        "description": "Attempt to add numbers without an auth token, expect 401",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "endpoint": "/add",
        "method": "POST",
        "description": "Add two negative numbers and verify the result is correct",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "endpoint": "/subtract",
        "method": "POST",
        "description": "Subtract b from a with valid auth and verify the result",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    {
        "name": "subtract_no_auth",
        "category": "INVALID_INPUT",
        "endpoint": "/subtract",
        "method": "POST",
        "description": "Attempt subtraction without auth token, expect 401",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "endpoint": "/multiply",
        "method": "POST",
        "description": "Multiply two numbers with valid auth and verify the result",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "endpoint": "/multiply",
        "method": "POST",
        "description": "Multiply a number by zero and verify the result is 0",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "endpoint": "/divide",
        "method": "POST",
        "description": "Divide a by b with valid auth and verify the result",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "endpoint": "/divide",
        "method": "POST",
        "description": "Attempt division without auth token, expect 401",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "method": "POST",
        "description": "Create a new calculation record with valid auth and operation",
        "request_data": {
            "path": {},
            "query": {},
            "body": {
    ...
    },
    {
        ...
        "endpoint": "/calculations",
        "method": "GET",
        "description": "List all stored calculations with valid auth, expect 200 and array response",
        "request_data": {
            "path": {},
            "query": {},
            "body": null,
    ...
    },
    {
        ...
        "endpoint": "/calculations",
        "method": "GET",
        "description": "Attempt to list calculations without auth, expect 401",
        "request_data": {
            "path": {},
            "query": {},
            "body": null
    ...
    },
    {
        ...
        "endpoint": "/calculations/{calculation_id}",
        "method": "GET",
        "description": "Attempt to get a calculation with a non-existent ID, expect 404",
        "request_data": {
            "path": {
...
def update_auth_from_response(
    ...
    explicit_auth_set = False
    if store_auth and isinstance(store_auth, dict):
        # Update headers (supports {placeholder} interpolation from _response_store and response)
        for header_name, template in store_auth.get("headers", {}).items():
            resolved = str(template)
            # Resolve {key} placeholders from response store first
            for key, value in _response_store.items():
                resolved = resolved.replace(f"{{{key}}}", str(value))
            # Also resolve {key} directly from response JSON fields
            if isinstance(response_json, dict):
                for key, value in response_json.items():
                    if isinstance(value, (str, int, float, bool)):
                        resolved = resolved.replace(f"{{{key}}}", str(value))

            # Skip headers with unresolved {key} placeholders — setting a literal
            # template value like "Bearer {access_token}" poisons all subsequent requests
            if re.search(r"\{[a-zA-Z_]\w*\}", resolved):
                print(
                    f"  Warning: auth header '{header_name}' has unresolved placeholders, "
                    f"skipping: {resolved}"
                )
                continue

            _auth_cache.setdefault("headers", {})[header_name] = resolved
            explicit_auth_set = True
        ...
        # Explicit store_auth failed to produce valid auth — fall through to auto-detect
        print("  Warning: store_auth did not resolve to valid auth, trying auto-detect")

    # --- Auto-detect auth from SETUP/AUTH category tests or failed store_auth ---
    if isinstance(response_json, dict):
...
def make_request(
    ...
    Args:
        content_type: Controls body encoding. Use "form" or
            "application/x-www-form-urlencoded" to send form data (e.g. OAuth2
            token endpoints). Defaults to JSON.
    """
    ...
    # Determine body encoding: form-urlencoded vs JSON
    is_form = bool(content_type and "form" in content_type.lower())
    ...
    if method == "GET":
        ...
        return requests.post(url, headers=headers, cookies=cookies, timeout=REQUEST_TIMEOUT, **body_kwargs)
    elif method == "PUT":
        return requests.put(url, headers=headers, cookies=cookies, timeout=REQUEST_TIMEOUT, **body_kwargs)
    elif method == "PATCH":
        return requests.patch(url, headers=headers, cookies=cookies, timeout=REQUEST_TIMEOUT, **body_kwargs)
    elif method == "DELETE":
        return requests.delete(url, headers=headers, cookies=cookies, timeout=REQUEST_TIMEOUT, **body_kwargs)
    else:
        return requests.request(method, url, headers=headers, cookies=cookies, timeout=REQUEST_TIMEOUT, **body_kwargs)
...
def run_setup(setup_config: dict[str, Any]) -> str | None:
    ...
    setup_content_type = setup_config.get("content_type")

    resp = make_request(method, endpoint, {}, {}, body, content_type=setup_content_type)
...
def get_fresh_tokens() -> dict[str, Any]:
    ...
    try:
        ...
        if login_cases:
            ...
            request_data = login_case.get("request_data", {})
            body = request_data.get("body", {})
            login_content_type = request_data.get("content_type")

            url = f"{BASE_URL.rstrip('/')}{endpoint}"
            # Support form-urlencoded login (e.g. OAuth2 token endpoints)
            if login_content_type and "form" in login_content_type.lower():
...
def test_api_endpoint(test_case: dict[str, Any]) -> None:
    ...
    try:
        ...
        try:
            resp = make_request(method, endpoint, path_params, query_params, body, req_headers, skip_auth, content_type)
